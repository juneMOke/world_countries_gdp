...
public class CountryDAO {
    ...
    public List<Country> getCountries(Map<String, Object> params) {
        ...
        return namedParamJdbcTemplate.query(SELECT_CLAUSE
                + " WHERE 1 = 1 "
                + (StringUtils.hasLength((String) params.get("search")) ? SEARCH_WHERE_CLAUSE : "")
                + (StringUtils.hasLength((String) params.get("continent")) ? CONTINENT_WHERE_CLAUSE : "")
                + (StringUtils.hasLength((String) params.get("region")) ? REGION_WHERE_CLAUSE : "")
                + PAGINATION_CLAUSE, params, new CountryRowMapper());
    }
    ...
    public int getCountriesCount(Map<String, Object> params) {
        return namedParamJdbcTemplate.queryForObject("SELECT COUNT(*) FROM country c"
                ...
                + (StringUtils.hasLength((String) params.get("search")) ? SEARCH_WHERE_CLAUSE : "")
                + (StringUtils.hasLength((String) params.get("continent")) ? CONTINENT_WHERE_CLAUSE : "")
                + (StringUtils.hasLength((String) params.get("region")) ? REGION_WHERE_CLAUSE : ""), params, Integer.class);
    }
